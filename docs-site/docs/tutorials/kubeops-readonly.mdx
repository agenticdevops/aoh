...
This mirrors [`docs/demos/kubeops-readonly.md`](https://github.com/agenticdevops/aoh/blob/main/docs/demos/kubeops-readonly.md)
...
- a `ClusterRole` named `aoh-readonly` granting `get`/`list`/`watch` on a fixed
  resource **allowlist** (nodes, pods, pods/log, events, endpoints, services,
  PVCs/PVs, namespaces, deployments/replicasets/daemonsets/statefulsets, jobs/
  cronjobs, metrics) — not a wildcard, and `secrets` is not on that list
...
```text
Error from server (Forbidden): pods "coredns-668d6bf9bc-kw88m" is forbidden: User "system:serviceaccount:default:aoh-kubeops-sresquad" cannot delete resource "pods" in API group "" in the namespace "kube-system"
```
...
:::tip[Honesty note]
Read-only is not read-nothing, but the generated `ClusterRole` is an explicit
resource allowlist, not a wildcard grant — `secrets`, `configmaps`, `nodes/proxy`,
`pods/exec`, `pods/attach`, `pods/portforward`, `serviceaccounts/token`, RBAC
objects, and `certificatesigningrequests` are all deliberately excluded, shared
across every adapter via `src/aoh/adapters/_k8s.py`. Live-verified against a real
cluster: `kubectl --kubeconfig $KC auth can-i get secrets` → `no`. Full matrix in
...
- **Enforcement lives in the target platform**, not the agent runtime — the same
  RBAC identity and guardrail carry over unchanged to the Claude Code and Codex
  adapters, both shipped now, pointed at this same pack; runtime knowledge never
  leaks into the guardrail itself. See
  [A Read-Only Kubernetes Agent with Claude Code](./kubeops-claude-code) for the
  Claude Code version, including its extra `permissions.deny` + hook guardrail
  layer.
