...
def generate_hermes_adapter(pack: Pack, output_dir: Path | str) -> AdapterResult:
    ...
    for workflow in pack.workflows:
        command_file = commands_dir / f"{workflow}.md"
        command_file.write_text(_render_command(pack, workflow), encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "pack": pack.name,
                "skills": pack.skills,
                "commands": [f"commands/{workflow}.md" for workflow in pack.workflows],
...
def install_hermes_pack(
    ...
    *,
    ...
    for skill in selected_skills:
        ...
        reference = destination / "references" / "aoh-workflow.md"
        reference.parent.mkdir(parents=True, exist_ok=True)
        reference.write_text(_render_workflow_reference(pack), encoding="utf-8")
...
def _render_command(pack: Pack, workflow: str) -> str:
    skills = ", ".join(f"`{skill}`" for skill in pack.skills) or "the generated AOH skills"
    ...
    return (
        f"# {workflow}\n\n"
        f"Use the {skills} skill for this AOH workflow.\n\n"
        f"- AOH pack: `{pack.name}`\n"
        f"- Agent role: {agent_roles}\n"
        f"- Model profile: {models}\n"
        f"- Runtime requirements: {requirements}\n\n"
        "Follow the skill instructions, keep the workflow focused, and report any runtime "
        "capability that Hermes cannot satisfy directly.\n"
    )


def _render_workflow_reference(pack: Pack) -> str:
    workflows = ", ".join(pack.workflows) or "none"
    ...
    return (
        f"# AOH Workflow Metadata: {pack.name}\n\n"
        ...
        "the pack's workflow context nearby.\n"
    )
...
def _render_agent_soul(pack: Pack, *, role: AgentRole | None = None) -> str:
    ...
    return (
        ...
        f"- Workflows: {workflows}\n"
        f"- Runtime requirements: {requirements}\n\n"
        "If the user asks for this pack's workflow, load and follow the associated "
