...
| 2026-07-19 | `promote_skill`'s conflict detection is a fresh `fetch_default_branch` at the start of every promote plus a fast-forward-only push at the end — not a persisted cross-session "last known base" ledger | Simpler than a persisted base and gives the same guarantee for the realistic case (one operator, occasional promotes); a stored multi-skill base ledger is deferred to Phase D if concurrent multi-operator promotion turns out to need it (YAGNI for v1) |
| 2026-07-19 | `skillcopy.copy_skill_tree` hygiene limits: `MAX_FILE_BYTES` 10 MiB/file, `MAX_TOTAL_BYTES` 50 MiB/skill, `MAX_FILE_COUNT` 500 files — enforced in a single pre-scan pass before anything is copied | A single oversized/huge draft must not partially land in the worktree (no partial copy on any violation) or bloat the pack repo silently; hard caps sized generously above any real skill (SKILL.md + a few scripts) while still catching mistakes (an accidentally-included dataset, log dump, or binary) |
| 2026-07-19 | A worktree cut from a `--mirror` clone inherits an `origin` remote with `remote.origin.mirror = true`, which rejects any push using an explicit refspec by NAME; `push_fast_forward` overrides this inline per-push (`-c remote.origin.mirror=false`) against the mirror, while every push to the REAL repo goes by URL (`gitops.push_branch`/`--pr` path), never by the remote name, even after `set_remote_url` has repointed `origin` at the real repo | Found via TDD while implementing Task 1/3 — pushing by name after repointing looked safe but the mirror-inherited remote config still forbids the refspec form; recording this so a future gitops.py extension doesn't silently reintroduce a push that only works by accident on some git versions |
| 2026-07-19 | `collections/core/aoh-authoring`'s skill invokes `aoh skill promote` via a thin deterministic script (`scripts/promote.sh` → `exec aoh skill promote "$@"`), not inline shell commands written into SKILL.md | Settles the engine-neutral question for any future skill that needs to call the `aoh` CLI itself: AOH still never executes anything — the skill is agent-authored guidance a runtime interprets, identical in kind to any other skill telling an agent to run `kubectl`/a script; a script wrapper keeps the skill declarative and testable (mode-checked, invocable directly) rather than hard-coding a shell invocation inline |
